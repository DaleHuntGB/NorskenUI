...
GUIFrame:RegisterContent("CustomSkin_Buffs", function(scrollChild, yOffset)
    ...
    local enableCheck = GUIFrame:CreateCheckbox(row1, "Enable Custom Buff Frame", db.Enabled ~= false,
        function(checked)
            db.Enabled = checked
    ...
    local iconSizeSlider = GUIFrame:CreateSlider(row2a, "Icon Size", 16, 80, 1, db.IconSize or 36, nil,
        function(value)
            db.IconSize = value
    ...
    -- Icon Spacing slider
    local iconSpacingSlider = GUIFrame:CreateSlider(row2a, "Icon Spacing", 0, 10, 1, db.IconSpacing or 1, nil,
        function(value)
    ...
    local iconsPerRowSlider = GUIFrame:CreateSlider(row2b, "Icons Per Row", 1, 20, 1, db.IconsPerRow or 12, nil,
        function(value)
    ...
    -- Max Rows slider
    local maxRowsSlider = GUIFrame:CreateSlider(row2b, "Max Rows", 1, 10, 1, db.MaxRows or 5, nil,
        function(value)
            db.MaxRows = value
    ...
    local enchantColorPicker = GUIFrame:CreateColorPicker(row3b, "Weapon Enchant Border", db.EnchantBorderColor,
        function(r, g, b, a)
    ...
    local fontSizeSlider = GUIFrame:CreateSlider(row4b, "Count Font Size", 8, 24, 1, db.FontSize or 12, nil,
        function(value)
            db.FontSize = value
            ApplySettings()
        end)
    ...
    -- Timer Font Size
    local timerFontSizeSlider = GUIFrame:CreateSlider(row4b, "Timer Font Size", 8, 32, 1, db.TimerFontSize or 14, nil,
        function(value)
    ...
    local includeEnchantsCheck = GUIFrame:CreateCheckbox(row5a, "Include Weapon Enchants",
        db.IncludeWeaponEnchants ~= false,
        function(checked)
    ...
    local sortMethodDropdown = GUIFrame:CreateDropdown(row5b, "Sort Method", sortMethodList, db.SortMethod, 30,
        function(key)
            db.SortMethod = key
    ...
    local sortDirDropdown = GUIFrame:CreateDropdown(row5b, "Sort Direction", sortDirList, db.SortDirection, 30,
        function(key)
