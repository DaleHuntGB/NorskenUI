...
GUIFrame:RegisterContent("CharacterPanel", function(scrollChild, yOffset)
    ...
    -- Card 2: Gem Socket Helper
    local gemDb = db.GemSocketHelper
    local card2 = GUIFrame:CreateCard(scrollChild, "Gem Socket Helper", yOffset)
    ...
    -- Card 3: Track Indicators
    local trackDb = db.TrackIndicators
    local card3 = GUIFrame:CreateCard(scrollChild, "Item Track Indicators", yOffset)
    manager:Register(card3, "all")

    local row6 = GUIFrame:CreateRow(card3.content, Theme.rowHeightLast)
    local trackEnableCheck = GUIFrame:CreateCheckbox(row6, "Show Item Track Letters", {
    ...
    row6:AddWidget(trackEnableCheck, 1)
