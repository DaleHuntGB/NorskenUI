...
GUIFrame:RegisterContent("BurningRush", function(scrollChild, yOffset)
    ...
    local card1 = GUIFrame:CreateCard(scrollChild, "Burning Rush Tracker", yOffset)

    local row1 = GUIFrame:CreateRow(card1.content, Theme.rowHeight)
    local enableCheck = GUIFrame:CreateCheckbox(row1, "Enable Burning Rush Tracker", {
        value = db.Enabled,
    ...
    previewBtn = GUIFrame:CreateButton(row1, "Show Preview", {
        ...
        callback = function()
            if BURN and BURN.TogglePreview then
                local isActive = BURN:TogglePreview()
